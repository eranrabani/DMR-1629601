...
long calcRadiativeLifetimesElecDipoleApprox(vector *intElecDipMoment, vector *elecDipoleMoment, intTwoQPState *intTwoQP,
	...
	double tmp, beta, temperature, totalRadiativeLifetime, boltzWeightedRadiativeRate, boltzWeightedRadiativeLifetime;
	...
	for (iNonintTwoQP = 0; iNonintTwoQP < lPar.nNonintTwoQPStates; iNonintTwoQP++) {
		...
		for (i = (iBin - nGaussianBins/2); i <= (iBin + nGaussianBins/2); i++) {
			if (i < nEmissionBins) {
				...
				absorptionIntensity[i] += (1.0/totalRadiativeLifetime * emissionGuassianPrefactor * exp(-0.5*sqr(dE/emissionBroadeningSigma))); 
